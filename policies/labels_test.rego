...
test_labels_cluster_missing_environment if {
	values := cluster_values_with_labels({
		"project": "p",
...
test_labels_cluster_staging_allowed if {
	values := cluster_values_with_labels({
		"environment": "staging",
		"project": "p",
		"managed_by": "terraform",
...
test_labels_cluster_invalid_environment if {
	values := cluster_values_with_labels({
		"environment": "testing",
		"project": "p",
		"managed_by": "terraform",
	...
	result := deny with input as cluster_plan(values)
	count(result) == 1
	result[_] == "Cluster google_container_cluster.primary has invalid environment label: testing"
}

test_labels_node_pool_production_allowed if {
	values := object.union(compliant_pool_values, {
		"node_config": [{
			"machine_type": "e2-small",
			"image_type": "COS_CONTAINERD",
...
test_labels_node_pool_invalid_environment if {
	values := object.union(compliant_pool_values, {
		"node_config": [{
			"machine_type": "e2-small",
			"image_type": "COS_CONTAINERD",
	...
	result := deny with input as pool_plan(values, true)
	count(result) == 1
	result[_] == "Node pool google_container_node_pool.primary_nodes has invalid environment label: testing"
}
